...
- 角度验证：拖动滑块把中间的圆形图转回正立方向
- 刮刮乐：拖动滑块从左往右横扫蒙版，提示图形全部出现后立即停止
- 曲线绘制：沿引导曲线从绿色起点描绘到红色终点
- 滑动曲线：大图上曲线两端固定，拖动滑块让曲线摆动对准真凹槽（含假凹槽干扰）
- 滑块摆动图块：拖动滑块让小图块沿多阶贝塞尔曲线运动到目标凹槽，方向随路径摆动（含假凹槽干扰）
...
- **统一模板 + 分类型实现**：行为校验由 `AbstractBehaviorValidator` 定义通用流程，滑块/点选/旋转/角度/刮刮乐/曲线/滑动曲线/滑块摆动各自实现事件序列与答案关联校验

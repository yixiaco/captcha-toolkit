...
/**
 * 刮刮乐行为校验：与滑块拖拽一致，轨迹必须是
 * “按下 → 连续移动 → 松开”的单次拖拽序列，且松开点的归一化 x
 * 必须与提交的滑块位置（横扫揭开的进度）一致。
 */
public class ScratchBehaviorValidator extends AbstractBehaviorValidator {

    /**
     * @param config 行为校验配置（含分端画像）
     */
    public ScratchBehaviorValidator(BehaviorConfig config) {
        super(config);
    }
    ...
    protected Optional<String> validateEvents(BehaviorTrace trace) {
        ...
        if (points.getFirst().type() != BehaviorEventType.START) {
            return Optional.of(CaptchaMessages.SCRATCH_EXPECTED_START);
        }
        if (points.getLast().type() != BehaviorEventType.UP) {
            return Optional.of(CaptchaMessages.SCRATCH_EXPECTED_RELEASE);
        }
        boolean hasMove = false;
        for (BehaviorPoint point : points) {
            if (point.type() == BehaviorEventType.DOWN) {
                return Optional.of(CaptchaMessages.SCRATCH_CLICK_NOT_ALLOWED);
            }
            hasMove |= point.type() == BehaviorEventType.MOVE;
        }
        if (!hasMove) {
            return Optional.of(CaptchaMessages.SCRATCH_MISSING_MOVE);
        }
        return Optional.empty();
    }

    /** 校验轨迹终点归一化 x 与提交的滑块位置一致 */
    @Override
    protected Optional<String> validateAnswer(
        ...
        if (answer == null || answer.getXNorm() == null) {
            return Optional.of(CaptchaMessages.SCRATCH_MISSING_X_NORM);
        }
        BehaviorPoint end = trace.points().getLast();
        if (Math.abs(end.x() - answer.getXNorm()) > profile.getPointTolerance()) {
            return Optional.of(CaptchaMessages.SCRATCH_END_MISMATCH);
        }
        return Optional.empty();
    }
}

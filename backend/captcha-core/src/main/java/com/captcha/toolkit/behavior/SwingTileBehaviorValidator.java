...
/**
 * 滑块摆动图块行为校验：轨迹必须是“按下 → 连续移动 → 松开”的拖拽序列，
 * 且松开点的归一化 x 必须与提交的滑块位置答案一致。
 */
public class SwingTileBehaviorValidator extends AbstractBehaviorValidator {

    /**
     * @param config 行为校验配置（含分端画像）
     */
    public SwingTileBehaviorValidator(BehaviorConfig config) {
        super(config);
    }
